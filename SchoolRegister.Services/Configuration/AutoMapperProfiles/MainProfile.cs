...
public class MainProfile : Profile
{
    public MainProfile()
    {
        ...
        CreateMap<Group, GroupVm>()
            .ForMember(dest => dest.Students, x => x.MapFrom(src => src.Students ?? new List<Student>()))
            .ForMember(dest => dest.Subjects, x => x.MapFrom(src => src.SubjectGroups != null ? src.SubjectGroups.Select(s => s.Subject) : new List<Subject>()));
