...
public class AccountApiController : BaseApiController
{
    ...
    public async Task<IActionResult> Login([FromForm] LoginUserVm applicationUser)
    {
        try
        {
            ...
            var result = await _signInManager.PasswordSignInAsync(applicationUser.Login, applicationUser.Password, false, false);
            if (!result.Succeeded)
            {
            ...
            var claims = new List<Claim>
            {
                ...
                new Claim(JwtRegisteredClaimNames.Exp, ((long)((DateTime.Now.AddMinutes(_jwtOptions.TokenExpirationMinutes) - new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)).TotalSeconds)).ToString())
            };
